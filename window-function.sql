...
SELECT constituency,party FROM (SELECT constituency,party, votes, RANK() OVER (PARTITION BY constituency ORDER BY votes DESC) AS pson
FROM ge
...
SELECT party, COUNT(1) FROM (SELECT constituency,party FROM (SELECT constituency,party, votes,RANK() OVER (PARTITION by constituency ORDER BY votes DESC) AS pson
  FROM ge
 WHERE constituency LIKE 'S%'
   AND yr  = 2017
ORDER BY pson, constituency) x
WHERE x.pson = 1) y
GROUP BY y.party;
